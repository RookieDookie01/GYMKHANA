const cmsURL="http://localhost/GYMKHANA/cms/";
const ajaxURL="http://localhost/GYMKHANA/cms/Requires/";
...
var pop = {
    ...
    updateStatus:function(id,value,This){
        swalWithBootstrapButtons.fire({
            title: "Are you sure?",
            text: "Did you confirm to Update this payment to "+value+"?",
            icon: "warning",
            showCancelButton: true,
...
function changePaymentStatus(This){
    var td = This.parents("td"), In_val = td.find( ".status" ).data("value"),new_val= td.find( ".select_status" ).val(), update = true;
    if(In_val == new_val){
        update = false;
        closeSelect(This);
        return;
    ...
    if(update){
        pop.updateStatus(This.data("id"),new_val,This);

    }
    
}

function updateStatus(value,id,This){
    var form= new FormData();
    form.append('id',id);
    form.append('value',value);
    $.ajax({
        url: ajaxURL+"updateStatus.php",
        method: 'POST',
        dataType: 'json',
        processData: false, //dont remove dont process the data
        contentType: false, // Let jQuery set the content type
        ...
        success: function(response) {
            if(response['responce']=="200"){
                pop.success("The Status Was Update Successful");
                changeText(This,value);
            }else{
                pop.fail(response['error']);
                closeSelect(This);
            }
        },
        error: function(xhr, status, error) {
...
function changeText(This,value){
    var td = This.parents("td"),status=td.find(".status");
    status.removeClass();
    status.addClass(getStatusBadgeClass(value));
    status.data("value",value);
    status.html(value)
    closeSelect(This);
}
function closeSelect(This){
    This.parents("td").find(".badge").show();
    This.hide();
}
...
function getStatusBadgeClass(paymentStatus) {
    switch (paymentStatus) {
        ...
        case 'Pending':
            return 'badge bg-warning status';
        case 'Paid':
            return 'badge bg-info status';
        case 'Failed':
            return 'badge bg-danger status';
        case 'Refunded':
...
function setCommentStatus(id,callback){
    var form= new FormData();
    form.append('id',id);
    $.ajax({
        url: ajaxURL+"updateComment.php",
        method: 'POST',
        dataType: 'json',
        processData: false, //dont remove dont process the data
        contentType: false, // Let jQuery set the content type
...
$(document).ready(function(){
        ...
        if(value!=""){
            checkUser(type,value,id,function(result){
                if(result){
                    ...
                    $("#errormessageUser.errormessageUser").remove();
                }else{
                    if($("#errormessageUser.errormessageUser").length===0){
                        newElement=$("<span>").attr("id","errormessageUser").addClass("errormessageUser error invalid-feedback").text("The Username was Duplicated with our system");
            ...
            username=false;
            if($("#errormessageUser.errormessageUser").length===0){
                newElement=$("<span>").attr("id","errormessageUser").addClass("errormessageUser error invalid-feedback").text("The Username Cannot be Empty");
                $(".username").addClass("is-invalid").after(newElement);
            }else{
                $("#errormessageUser.errormessageUser").text("The Username Cannot be Empty");
                $(".username").addClass("is-invalid").after(newElement);
            }
        }
    });
    ...
    $(".status").on("dblclick",function(){
        var This = $(this), td = This.parents("td");
        This.hide();

        td.find( ".select_status" ).show();
        td.find( ".select_status" ).focus();
    });

    $(".select_status").on("blur",function(e){
        changePaymentStatus($(this));
    });
    $(".unreadComment").on("click",function(e){
        var This=$(this),tr = This.parents("tr"),status=tr.find(".commentstatus"),id=This.data('id');
        setCommentStatus(id,function(result){
            if(result){
                status.html("Read");
                status.removeClass("text-danger");
                status.addClass("text-success");

                This.removeClass("unreadComment");
            }
        });
    });
});
